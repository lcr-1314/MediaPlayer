...
public abstract class WlBaseMediaEncoder {
    ...
    private void initVideoEncodec(String mimeType, int width, int height) {
        try {
            ...
            videoFormat.setInteger(MediaFormat.KEY_FRAME_RATE, 30);//帧率FPS-指每秒显示的图片数，或者GPU每秒能够更新的次数
            ...
            // 这样MediaCodec 就会自动编码 surface中的图像
    ...
    private void initAudioEncodec(String mimeType, int sampleRate, int channelCount) {
        try {
            ...
            audioFormat = MediaFormat.createAudioFormat(mimeType, sampleRate, channelCount);//采样率及声道数
            audioFormat.setInteger(MediaFormat.KEY_BIT_RATE, 96000);//帧率
