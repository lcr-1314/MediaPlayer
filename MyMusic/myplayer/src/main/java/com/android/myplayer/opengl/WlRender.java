package com.android.myplayer.opengl;
...
public class WlRender implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener{
    private final static String TAG = "WlRender";
    ...
    public void onFrameAvailable(SurfaceTexture surfaceTexture) {
        if (onRenderListener != null) {
            onRenderListener.onRender(); // 调用手动刷新
        }
    }
    ...
    private void initRenderYUV() {
        ...
        avPosition_yuv = GLES20.glGetAttribLocation(program_yuv, "av_Position"); // 顶点坐标
    ...
    private void initRenderMediacodec() {
        ...
        avPosition_mediacodec = GLES20.glGetAttribLocation(program_mediacodec, "av_Position");  //顶点句柄
        afPosition_mediacodec = GLES20.glGetAttribLocation(program_mediacodec, "af_Position");  //纹理句柄
        samplerOES_mediacodec = GLES20.glGetUniformLocation(program_mediacodec, "sTexture");    //扩展纹理句柄

        int[] textureids = new int[1];
        GLES20.glGenTextures(1, textureids, 0);
        textureId_mediacodec = textureids[0];   // 硬解码纹理Id
        ...
        surfaceTexture = new SurfaceTexture(textureId_mediacodec); // 绑定硬解码Id到surface.
