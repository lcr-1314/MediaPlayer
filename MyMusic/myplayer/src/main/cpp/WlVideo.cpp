...
WlVideo::WlVideo(WlPlaystatus *playstatus, WlCallJava *wlCallJava) {
    ...
    this->queue = new WlQueue(playstatus);
...
void *playVideo(void *data){
    ...
    while (video->playstatus != NULL && !video->playstatus->exit) {
        ...
        if (video->queue->getQueueSize() == 0) { // 没有数据，则循环等待
        ...
        if (video->codectype == CODEC_MEDIACODEC) { // 硬解码。
            ...
            while (av_bsf_receive_packet(video->abs_ctx, avPacket) == 0) {
                ...
                av_usleep(video->getDelayTime(diff) * 1000000); // 睡眠达到音频和视频同步播放
            ...
            if (avFrame->format == AV_PIX_FMT_YUV420P) {
                ...
                LOGE("pFrameYUV420P->pts: %d", avFrame->pts);
                ...
                video->wlCallJava->onCallRenderYUV(
                        ...
                        avFrame->data[1],
                        avFrame->data[2]);
            } else { // 当前视频不是YUV420P格式的，需要转码成YUV420P格式的
                ...
                LOGE("pFrameYUV420P->pts: %d", pFrameYUV420P->pts);
...
double WlVideo::getDelayTime(double diff) {
    if (diff > 0.003) {
        ...
        if (delayTime < defautDelayTime / 2) {
            delayTime = defautDelayTime * 2 / 3;
        } else if (delayTime > defautDelayTime * 2) {
            delayTime = defautDelayTime * 2;
        }
    } else if (diff < -0.003) {
